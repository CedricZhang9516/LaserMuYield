...
//static constexpr int nRhos = 5; //number of density matrix (and ionization rate) parameters 
static constexpr int nRhos = 17; //number of density matrix (and ionization rate) parameters
...
class Muonium
{
 public:
  ...
  static double GetIonizationRate2P( double intensity /*[W/mm^2]*/ ) { return 2*M_PI * beta_ioni_2P * intensity; }; // [rad/s] 2P1/2, 2P3/2, (Laser Physics, Vol. 12, No. 7, 2002, pp. 1–8.)
  static double Get2ndDoppler( double v/*[mm/s]*/ ) { return Omega_eg/2 * pow(v/2.99792458e11,2); };             // [rad/s] (PRA73,052501,2006 Eq.12)
  static double Get1stDoppler( std::array<double,3> Vel ) { return Omega_eg * Vel[0]/2.99792458e11; };           // [rad/s], laser advance in x direction (Demtroder, Laser spectroscopy Vol. 2 (9.3))
  ...
  static double GetACStarkShift2P( double intensity /*[W/mm^2]*/) { return 2*M_PI * beta_ac_2P * intensity; };   // [rad/s] for 244 nm (tentatively used for also 355 nm)
  ...
  static constexpr double ProtonMass   = 938.272081;  // MeV/c^2 (PDG2018)
  ...
  static constexpr double Omega_2S2P1_2 = 2*M_PI * 1.04703e9; // rad/s (Physics Letters B Volume 44, Issue 2, 16 April 1973, Pages 199-201)
  static constexpr double Omega_2S2P3_2 = 2*M_PI * 9.87447e9; // rad/s (Physics Letters B Volume 44, Issue 2, 16 April 1973, Pages 199-201)

  static constexpr double GammaS     = 2*M_PI * 1.31; // rad/s (2S spontaneous decay, PRA73,052501,2006, Table1) (actually negligibly small)
  static constexpr double GammaDecay = 2*M_PI * 72.442e3; // rad/s (FWHM)

  static constexpr double GammaP = 2*M_PI * 625.0e6 * MuonMass*(ElectronMass+ProtonMass)/ProtonMass/(ElectronMass+MuonMass); // rad/s (FWHM) (hydrogen 2P spontaneous decay (1.6 ns), Phys. Rev.148.1)
  ...
  static constexpr double beta_ac_2P = beta_ac_2S * 7.37 / 8.81; // [Hz/(W/mm^2)] (Laser Physics, Vol. 12, No. 7, 2002, pp. 1–8.)
  static constexpr double beta_ioni_2P = beta_ioni * 5.93 / 7.56; // [Hz/(W/mm^2)] (Laser Physics, Vol. 12, No. 7, 2002, pp. 1–8.)

  static constexpr int GetnRhoparams( void ) {return nRhos;};

 private:
  std::array<double,3> fStartPos; // (X_sf, Y_sf, Z_sf)
  double fStartTime;   // ( t )
  std::array<double,3> fStartVel; // (VX_sf, XY_sf, VZ_sf)
  std::array<double,nRhos> fRho;  
